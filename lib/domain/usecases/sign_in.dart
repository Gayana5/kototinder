...
class SignInUseCase {
  SignInUseCase(this._repository, this._validator, this._analytics);
  ...
  Future<Result<void>> call({required String email, required String password}) async {
    ...
    if (emailError != null) {
      await _analytics.logEvent('login_error', parameters: {'reason': 'email'});
      return Result.failure(emailError);
    }
    if (passwordError != null) {
      await _analytics.logEvent('login_error', parameters: {'reason': 'password'});
      return Result.failure(passwordError);
    }
    final result = await _repository.signIn(email: email.trim(), password: password);
    if (result.isSuccess) {
      await _analytics.logEvent('login_success');
    } else {
      await _analytics.logEvent('login_error', parameters: {'reason': 'credentials'});
    }
    return result;
  }
}
